...
# Run this code for 1 target simulation
...
param <- read.csv("data/PSA_1target/EndOfGenAPMC__AllParamSets_Run#601.csv")
outcome <-read.csv("data/PSA_1target/EndOfGenAPMC__AllSimnOutcome_Run#601.csv")
for (runID in runID_list){
  this_param <- read.csv(paste0("data/PSA_1target/EndOfGenAPMC__AllParamSets_Run#",paste0(runID,".csv")))
  param <-rbind(param,this_param)
  this_outcome <- read.csv(paste0("data/PSA_1target/EndOfGenAPMC__AllSimnOutcome_Run#",paste0(runID,".csv")))
...
y_targets_se <- matrix(0.05,ncol=16,nrow=1) #t(as.matrix(y_targets_se)) #matrix(0.1,ncol=16,nrow=1) 
...
png(filename='output/ann_convergence_influenza_1targ.png')
plot(history)
dev.off()
# plot ANN validation
...
png(filename='output/ann_validation_vs_observed_influenza_1targ.png')
...
write.csv(Xq_unscaled, file = "output/calibrated_posteriors_influenza_NewBayCANN_MSE_500k.csv")
write.csv(exp(pred_y_unscaled), file = "output/posterior_predicted_influenza_NewBayCANN_MSE_500k.csv")
