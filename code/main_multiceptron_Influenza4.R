...
# Original code: main_multiceptron_Influenza3.R
# Change made: 
# (1) Fit ANN to two targets instead of one. (binary outcome that indicates whether the influenza peak happened before/after Feb 28)
...
outcome2 <-read.csv("data/PSA_2targets/EndOfGenAPMC__AllSimnPeakAll_Run#700.csv")
...
outcome <- cbind(outcome1, outcome2)
colnames(param) <- c("trans.T1","trans.T2","trans.T3","trans.T4","immun1","immun2","immun3","astn12","astn13","astn14","astn15")
colnames(outcome) <- c("rate.T1.12","rate.T2.12","rate.T3.12","rate.T4.12","rate.T1.13","rate.T2.13","rate.T3.13","rate.T4.13","rate.T1.14","rate.T2.14","rate.T3.14","rate.T4.14","rate.T1.15","rate.T2.15","rate.T3.15","rate.T4.15","peak") 
Xunscaled <- param
Yunscaled <- cbind(log(outcome[,1:ncol(outcome1)]),outcome[,ncol(outcome)])# transformation of outcome data. outcome
