...
# Original code: main_multiceptron_Influenza4_2BayCANN_unscaled.R
# Change made: 
# (1) ANN is fitted to the samples that peak earlier than 365
# (2) estimate SE as random
...
# remove the samples peak == 366 (max)
keep_idx <- which(outcome[,17]<366&outcome[,18]<366&outcome[,19]<366&outcome[,20]<366)
...
s0 <- stan(file = "code/post_multi_perceptron_influenza_peak_unscale_randSE.stan", data = stan.dat, iter = n_iter, chains = n_chains, 
...
print(s0)
plot(s0)
traceplot(s0,inc_warmup=TRUE, pars=c('Xq','y_targets_se'))
