# Code last updated on: 2021. 02.21
# Original code: main_multiceptron_Influenza4.R
# Change made: 
# (1) Run this code for 2-target simulation (1) 16 rates (2) peak dates
# (2) This code fits PSA samples to 1 ANN
# (3) in BayCANN, peak dates are transformed to  
# (4) targets are set to be 
...
y_targets_se <- matrix(0.005,ncol=ncol(y_targets),nrow=1) #t(as.matrix(y_targets_se)) #matrix(0.1,ncol=16,nrow=1) 
y_targets_se[17:20] = 0.005 # smaller se for peak dates
...
png(filename='output/ann_convergence_influenza_2targ_continuous.png')
plot(history)
dev.off()
# plot ANN validation
...
png(filename='output/ann_validation_vs_observed_influenza_2targ_continuous.png')
...
s0 <- stan(file = "code/post_multi_perceptron_influenza_peak.stan", data = stan.dat, iter = n_iter, chains = n_chains, 
           pars = c("Xq") , control = list(adapt_delta = 0.9,max_treedepth = 15))
s_df <- extract(s0, permuted = FALSE)
Xq  <- s_df$Xq
...
pred_y_unscaled_exp <- exp(pred_y_unscaled)
Xq_accept <- Xq_unscaled[(pred_y_unscaled_exp[,17]<138)&(pred_y_unscaled_exp[,18]<138)&(pred_y_unscaled_exp[,19]<138)&(pred_y_unscaled_exp[,20]<138),]
# SAve the unscaled posterior samples
write.csv(Xq_unscaled, file = "output/calibrated_posteriors_influenza_NewBayCANN_2targets_50k7chains.csv")
write.csv(exp(pred_y_unscaled), file = "output/posterior_predicted_influenza_NewBayCANN_2targets_50k7chains.csv")
